...
async def start(bot: DonLee_Robot_V2, msg: Import.Msg):
    START_BUTTON = [[  
          Import.Button("➕ 𝖠𝖽𝖽 𝖬𝖾 𝖳𝗈 𝖸𝗈𝗎𝗋 𝖢𝗁𝖺𝗍𝗌 ➕", url=f"http://t.me/Legend_boy_robot?startgroup=botstart")
          ],[
...
async def about(bot: DonLee_Robot_V2, msg: Import.Msg):
        button = [[
          Import.Button("🤓 𝘀𝘂𝗽𝗽𝗼𝗿𝘁 🤓", url='https://www.youtube.com/watch?v=NrbMc93aCzA'),
          Import.Button("𝘀𝗼𝘂𝗿𝗰𝗲 𝗰𝗼𝗱𝗲📦", callback_data="source")
          ],[
          Import.Button("⚠️𝗵𝗲𝗹𝗽", callback_data="help"),
          Import.Button("🏠𝗯𝗮𝗰𝗸", callback_data="home"),
          Import.Button("🗑️", callback_data="close")
          ]]                     
        await bot.send_photo(
            chat_id=msg.chat.id,
            photo=random.choice(Config.PHOTO),
...
async def admin(bot, msg):
    button = [[  
       Import.Button("𝖢𝗅𝗂𝖼𝗄 𝖧𝖾𝗋𝖾➡️", url="t.me/Alifmuhammad_tg")
       ]]
    await msg.reply_text(
